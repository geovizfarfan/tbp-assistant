...
async function runPayoutReminders(client) {
  ...
  const res = await query(
    `SELECT pr.*, s.channel_id as sched_channel
     FROM payout_reminders pr
  ...
  for (const reminder of res.rows) {
    ...
    try {
      ...
      await channel.send(
        `⏰${lateTag} <@${reminder.host_id}> reminder: <@${reminder.winner_id}> is waiting for **${reminder.prize}**.${adminMention ? `\n${adminMention}` : ''}`
      );
