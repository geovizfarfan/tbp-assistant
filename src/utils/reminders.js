...
async function runPayoutReminders(client) {
  ...
  for (const reminder of res.rows) {
    ...
    try {
      ...
      const lateTag = tier.markLate ? ' <a:atention:1512916995543273642> **LATE PAYOUT**' : '';
      await channel.send(
        `<a:RojasClock:1512912822613446787>${lateTag} <@${reminder.host_id}> reminder: <@${reminder.winner_id}> is waiting for **${reminder.prize}**.${adminMention ? `\n${adminMention}` : ''}`
      );
