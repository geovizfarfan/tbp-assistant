...
// new balance, or null if the adjustment failed.
async function adjustGuildBalance(guildId, userId, username, amount) {
  const { useSins } = await getGuildCurrencyConfig(guildId);

  if (useSins) {
    return adjustBalance(userId, username, amount).catch((err) => {
